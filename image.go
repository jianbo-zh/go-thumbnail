...
func ImageAndSave(fileInPath string, outputDir string) (*FileResult, error) {
	...
	if strings.HasPrefix(mimeType, "image") {
		...
		// 缩略图
		gocv.Resize(src, &dst, image.Point{}, 0.01, 0.01, gocv.InterpolationDefault) // 按原比例缩放， 缩放后的长 ,宽为 原来的25%
		//gocv.Resize(src, &dst, image.Pt(120, 68), 0, 0, gocv.InterpolationDefault) //两种缩放方式
		...
		for i := 0; i < 200; i++ {
			...
			if ok {
				...
				if !img.Empty() {
					...
					// 缩略图
					gocv.Resize(img, &dst, image.Point{}, 0.01, 0.01, gocv.InterpolationDefault)
